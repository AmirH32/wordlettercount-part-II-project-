...
"""
Parameters:
1. spark.executor.instances   [2, 10] (inclusive)
...
def decode(x):
    ...
    parallelism = int(
        np.clip(round(x[2]), PARAM_PARALLELISM["low"], PARAM_PARALLELISM["high"])
    )
...
def latin_hypercube_samples(n):
    ...
    for i_f, m_f, p_f in zip(inst_samples, mem_samples, par_samples):
        instances = round(
            PARAM_EXECUTOR_INSTANCES["low"]
            + i_f * (PARAM_EXECUTOR_INSTANCES["high"] - PARAM_EXECUTOR_INSTANCES["low"])
        )
        instances = int(
            np.clip(
                instances,
        ...
        mem_index = int(
            np.clip(
                round(m_f * (len(PARAM_EXECUTOR_MEMORY["choices_str"]) - 1)),
                0,
                len(PARAM_EXECUTOR_MEMORY["choices_str"]) - 1,
            )
        )
        parallelism = round(
            PARAM_PARALLELISM["low"]
            + p_f * (PARAM_PARALLELISM["high"] - PARAM_PARALLELISM["low"])
        )
        parallelism = int(
...
def next_candidate(gp, y_best, n_restarts=200):
    ...
    inst_cands = np.random.uniform(
        PARAM_EXECUTOR_INSTANCES["low"], PARAM_EXECUTOR_INSTANCES["high"], n_restarts
    )
    mem_cands = np.random.uniform(
        PARAM_EXECUTOR_MEMORY["low"], PARAM_EXECUTOR_MEMORY["high"], n_restarts
    )
...
def run_spark(input_file, instances, mem_str, parallelism):
    ...
    cmd = [
        ...
        APP_CLASS,
        "--conf",
        f"spark.executor.instances={instances}",
        "--conf",
        f"spark.executor.memory={mem_str}",
        "--conf",
        f"spark.default.parallelism={parallelism}",
        "--conf",
        ...
        "--conf",
        f"spark.kubernetes.driver.volumes.persistentVolumeClaim.{PVC_NAME}.mount.path={MOUNT_PATH}",
        "--conf",
        f"spark.kubernetes.driver.volumes.persistentVolumeClaim.{PVC_NAME}.mount.readOnly=false",
        "--conf",
        f"spark.kubernetes.driver.volumes.persistentVolumeClaim.{PVC_NAME}.options.claimName={PVC_NAME}",
        "--conf",
        f"spark.kubernetes.executor.volumes.persistentVolumeClaim.{PVC_NAME}.mount.path={MOUNT_PATH}",
        "--conf",
...
def write_dynamic_csv(timestamp, input_file, best):
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writerow(
            [f"# Total runs   : {len(results_log)}  ({N_INITIAL} LHS + {N_BO} BO)"]
        )
        writer.writerow(
            [
                f"# Best config  : instances={best['instances']}  "
...
def main():
    if len(sys.argv) != 2:
        print(
            "Usage: python3 dynamic_configuration_parameter_scheduler.py <input_file>"
        )
        sys.exit(1)
    ...
    print(
        f"Parameters : spark.executor.instances | spark.executor.memory | spark.default.parallelism"
    )
    print(f"{'=' * 65}\n")
    ...
    print(f"{'=' * 65}")
    print(
        f"{'Run':<6} {'Phase':<6} {'Instances':<12} {'Memory':<10} "
        f"{'Parallelism':<14} {'Time (s)':<10}"
    )
    print("-" * 58)
    for r in results_log:
        print(
            f"{r['run']:<6} {r['phase']:<6} {r['instances']:<12} "
